...
const ContactForm = () => {
    ...
    const SendEmail = (templateParams: any) => {
        ...
        emailjs.sendForm(emailServiceID, templateServiceID, templateParams, emailUserID).then(() => {
    ...
    const formikConfig = useFormik({
        ...
        validationSchema: Yup.object({
            ...
            message: Yup.string().required("It is required to fill out message field."),
        }),
        onSubmit: (values: any) => {
            SendEmail(values);
            setSendEmailSuccess(true)
        ...
        <Box overflow="hidden" style={{ position: 'relative', minHeight: '300px' }}>
                ...
                >
                    ...
                    <Typography
                        ...
                        variant="body2"
                    >
                        Your message has been sent successfully. I'll reply as soon as possible.
                    </Typography>
                </Box>
                ) : (
                        <form onSubmit={formikConfig.handleSubmit}>
                            <TextField
                                error={Boolean(formikConfig.touched.name && formikConfig.errors.name)}
                            ...
                            />
                            <TextField
                                error={Boolean(formikConfig.touched.email && formikConfig.errors.email)}
                                onBlur={formikConfig.handleBlur}
                                onChange={formikConfig.handleChange}
                            ...
                            />
                            <TextField
                                error={Boolean(formikConfig.touched.message && formikConfig.errors.message)}
                                onBlur={formikConfig.handleBlur}
                                onChange={formikConfig.handleChange}
                                value={formikConfig.values.message}
                                helperText={formikConfig.touched.message && formikConfig.errors.message}
                                variant="filled"
                                margin="normal"
                                fullWidth
                                name="message"
